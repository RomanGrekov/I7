...
#define LCD_PORT GPIOB //Порт к которому подключен LCD
#define LCD_CLOCK_REG RCC->APB2ENR //Регистр конфигурации тактирования периферии
#define LCD_CLOCK_BIT RCC_APB2ENR_IOPBEN// Бит включения тактирования порта B
#define LCD_CD 10 // Вообще обычно RS. Переключение команда(если 0)/данные(если 1).
#define LCD_EN 11  // Стробирующий бит
#define LCD_DB4 12 // Биты шины данных
#define LCD_DB5 13
#define LCD_DB6 14
#define LCD_DB7 15
///////////////////////////////////
...
typedef enum { // Включить/выключить экран
	LCD_DISABLE = 0,
	LCD_ENABLE = !LCD_DISABLE
} lcd_state;

typedef enum { // Включить/выключить курсор
	CURSOR_DISABLE = 0,
	CURSOR_ENABLE = !CURSOR_DISABLE
} cursor_state;

typedef enum { // Включить/выключить мигание курсором
	NO_BLINK = 0,
	BLINK = !NO_BLINK
} cursor_mode;
...
#define LCD_EN_BS (1 << LCD_EN) // Маска строба set (установить)
#define LCD_CD_BS (1 << LCD_CD) // Маска команда/данные set (установить)
#define LCD_DB4_BS (1 << LCD_DB4) // Маска битов данных set (установить)
#define LCD_DB5_BS (1 << LCD_DB5)
#define LCD_DB6_BS (1 << LCD_DB6)
#define LCD_DB7_BS (1 << LCD_DB7)

#define LCD_EN_BC (1 << (LCD_EN+16)) // Маска строба reset (сбросить)
#define LCD_CD_BC (1 << (LCD_CD+16)) // Маска команда/данные reset (сбросить)
#define LCD_DB4_BC (1 << (LCD_DB4+16)) // Маска битов данных reset (сбросить)
